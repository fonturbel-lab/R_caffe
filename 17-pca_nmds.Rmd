...
The non-metric multidimensional scaling works in a similar philosophy than `PCA`, grouping variables using an ordination algorithm. Usually we apply `nMDS` together with `ANOSIM` to visualize species composition differences (but it can be used with any kind of variables that have at least two discrete groups).
