...
Awesome! with this code we can determine that Cd and Zn are not different between sites, but the other metals present significant differences.
...
```{r mvn1}
multinorm<-mvn(y, mvn_test = "royston")
multinorm
par(mfrow=c(3,3))
for(i in 1:ncol(y)){
  qqnorm(y[,i], main = paste("QQ Plot -", names(y)[i]))
  qqline(y[,i], col = "blue")
}
par(mfrow=c(1,1))
```
...
```{r m0np}
manova0np<-adonis2(as.matrix(y)~Site)
manova0np
```
