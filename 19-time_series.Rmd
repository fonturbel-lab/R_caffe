...
m1.aphcan
#Diagnostico del modelo ARIMA
tsdiag(m1.aphcan)
Box.test(m1.aphcan$residuals,lag=1)
#Para calcular la significancia del coeficiente "ma1" hacemos esto:
(1-pnorm(abs(m1.aphcan$coef)/sqrt(diag(m1.aphcan$var.coef))))*2
# Diagnostico: Residuales del modelo ARIMA(0,1,1) (0,1,1)12
plot(window(rstandard(m1.aphcan),start=c(2003,2)), ylab='Standardized Residuals',type='o')
abline(h=0)
# ACF e histograma de los residuales del ARIMA
